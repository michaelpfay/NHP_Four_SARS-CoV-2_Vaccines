...
for (i in 1:ncol(combs.m)) {
  outcomes.m[which(immune.markers.log.m == combs.m[2, i]), which(immune.markers.log.m == combs.m[1, i])] <- cor_return(D.m[ ,combs.m[1, i]], D.m[ ,combs.m[2, i]])[1]
}
...
plot(as.vector(outcomes.m),as.vector(outcomes.f), xlab="male",ylab="female",main="Spearman correlations between Immune Markers")
...
plot(as.vector(outcomes.m),as.vector(outcomes.f), xlab="male",ylab="female",main="Spearman correlations between Viral Load Measurements")
lines(c(-1,1),c(-1,1),lty=2)
...
# Both 
combs <- t(expand.grid(cont.outcomes.log, immune.markers.log))
...
# Male 
combs.m <- t(expand.grid(cont.outcomes.log.m, immune.markers.log.m))
...
# Female 
combs.f <- t(expand.grid(cont.outcomes.log.f, immune.markers.log.f))
